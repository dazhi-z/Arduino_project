...
class SerialPort {
  public:
    ...
    // returned string. NOTE, the carriage return '\r' is ignored completely
    // Returns "" if it times out, i.e. '\n' is not read within the "timeout"
    // milliseconds If timeout <= 0, then no timeout will be used: it will wait
    // indefinitely until '\n' is read.
