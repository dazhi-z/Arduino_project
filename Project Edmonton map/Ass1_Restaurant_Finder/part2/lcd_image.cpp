 ...
 */
void lcd_image_draw(const lcd_image_t *img, MCUFRIEND_kbv *tft, uint16_t icol,
                    uint16_t irow, uint16_t scol, uint16_t srow, uint16_t width,
                    uint16_t height) {
    File file;
    ...
    for (uint16_t row = 0; row < height; row++) {
        ...
        // Seek to start of pixels to read from, need 32 bit arith for big
        // images
        uint32_t pos =
            ((uint32_t)irow + (uint32_t)row) * (2 * (uint32_t)img->ncols) +
            (uint32_t)icol * 2;
        file.seek(pos);
