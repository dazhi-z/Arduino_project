...
// the values of button and message in game for touching screen
...
// initialize the SD card and the screen
void setup() {
    init();
    Serial.begin(9600);
...
// create the random seed from the A7 pin on the Arduino
unsigned long getSeed() {
...
// delete the record in the sd card and write the new one
void updateRecord() {
    File myFile;
    SD.remove("stats.txt");
    myFile = SD.open("stats.txt", FILE_WRITE);
...
// delete the game record in the sd card and write the new one
void updateResume() {
    File myFile;
    if (SD.exists("game.txt")) {
        SD.remove("game.txt");
...
// The main menu is the main interface
void mainMenu() {
...
// When the sudoku game is finished, show the used time
// and go to the statistics interface.
void gameover() {
    record.run[diff]++;
...
// draw the cross on the given place
void drawCross(int16_t grid_x, int16_t grid_y) {
    tft.fillRoundRect(
        CROSS_SPACE + (BLOCK_SIZE + SPACE) * grid_x + (grid_x / 3 * 3),
        BLOCK_TOP_MARGIN + (BLOCK_SIZE + SPACE) * grid_y + (grid_y / 3 * 3),
        BLOCK_SIZE, BLOCK_SIZE, RADIUS, RED);
    tft.drawLine((BLOCK_LEFT_MARGIN + CROSS_SPACE) +
                     (BLOCK_SIZE + SPACE) * grid_x + (grid_x / 3 * 3),
                 (BLOCK_TOP_MARGIN + CROSS_SPACE) +
                     (BLOCK_SIZE + SPACE) * grid_y + (grid_y / 3 * 3),
                 (BLOCK_SIZE + SPACE) * grid_x + (grid_x / 3 * 3) + BLOCK_SIZE,
                 (BLOCK_TOP_MARGIN - CROSS_SPACE) +
                     (BLOCK_SIZE + SPACE) * grid_y + (grid_y / 3 * 3) +
                     BLOCK_SIZE,
                 TFT_RED);
    ...
    delay(300);
    tft.fillRoundRect(
        BLOCK_LEFT_MARGIN + (BLOCK_SIZE + SPACE) * grid_x + (grid_x / 3 * 3),
        BLOCK_TOP_MARGIN + (BLOCK_SIZE + SPACE) * grid_y + (grid_y / 3 * 3),
        BLOCK_SIZE, BLOCK_SIZE, RADIUS, RED);
}
...
// start a sudoku game
void startGaming() {
    int currDigit = 1; // the number chose currently
    CurrentTime = millis();
    while (true) {
        TSPoint touch = ts.getPoint();
        ...
        if (touch.z >= MINPRESSURE && touch.z <= MAXPRESSURE) {
            ...
            if (screen_x >= GAME_BOTTOM && screen_x <= GAME_TOP) {
                ...
                if (screen_y >= GAME_TIPS_LEFT && screen_y <= GAME_TIPS_RIGHT) {
                    ...
                    // if countResult returns true, it means the game is done
            ...
            screen_x -= 90;
            // grid_x and grid_y (range 0-8) refer to the grid
            // that is pointed
            int16_t grid_x = (310 - screen_y) / (BLOCK_SIZE + SPACE),
                    grid_y = (screen_x - 8) / (BLOCK_SIZE + SPACE);

            // go back to the loop if it is out of the range
            if (grid_y < 0 || grid_y > 8 || grid_x > 8 || grid_x < 0) {
                continue;
            }
            // all empty grids are modifiable
...
// Let the user choose the level to start a new
// game or resume the previous game
void processGame() {
    bool resume = false;
    // display the difficulty choosing menu
    difficultyMenu(resume);
    if (currentState == Menu) {
        return;
    }
    // if the user doesn't choose to resume the game, then initialize a new game
    if (!resume) {
        sudoku.newGame(diff);
    }
